...
신뢰의 핵심은 **해시 체인**과 **디지털 서명** 두 가지입니다.
...
단 하나의 레코드라도 수정되면 이후 모든 `prevHash`가 불일치하여
변조가 즉시 탐지됩니다.
...
| 환경 | 서명 방식 | 비고 |
|---|---|---|
| 소프트웨어 (개발) | SHA-256 소프트 서명 | 테스트 전용 |
| 하드웨어 (운영) | TPM AIK + Dilithium3 | UC-005 이후 적용 |
...
| 규칙 | 임계값 | 심각도 |
|---|---|---|
| 비정상 접근 빈도 | 10분 내 5회 이상 | HIGH |
| 해시/서명 불일치 | 1건이라도 발생 | CRITICAL |
| 연속 DENY | 3회 연속 | MEDIUM |
| 위임 없는 대리 접근 | 1건이라도 발생 | HIGH |
...
| 서비스 유형 | 의무 보존 기간 | 근거 |
|---|---|---|
| 금융 투자 (UC-002) | 5년 | 자본시장법 |
| 기업 구매 (UC-003) | 5년 | 법인세법 |
| 행정 서비스 (UC-004) | 5년 | 전자정부법 |
| 기타 | 1년 | 내부 정책 |
