...
구성 요소필수 여부최소 사양TPM 2.0권장 (SW 대체 가능)TCG TPM 2.0, 24개 PCRTEE권장 (SW 대체 가능)ARM TrustZone 또는 OP-TEESecure Boot권장UEFI 서명 검증NVMe 암호화권장AES-256 (TCG Opal 2.0)
2.2 소프트웨어 환경
구성 요소필수 여부사양AI Runtime필수Python 3.11+ 또는 동등 환경Policy Engine 클라이언트필수KorPIX SDK v0.1+Audit 기록 에이전트필수로컬 또는 원격 Audit Network 연결사용자 인증필수공동인증서 / PASS / 생체 중 하나
...
KorPIX Policy API v0.1 이상과 통신 가능
KorPIX Audit Protocol v0.1 이상 준수
ActionRecord JSON Schema 검증 통과
digitalSignature 필드 미포함 레코드 거부
